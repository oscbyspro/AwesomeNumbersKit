...
extension ANKFullWidth {
    ...
    @inlinable mutating func formQuotientReportingRemainder(dividingBy divisor: Digit) -> Digit {
        let qr = quotientAndRemainder(dividingBy: divisor); self = qr.quotient; return qr.remainder
    }
    
    @inlinable mutating func formRemainderReportingQuotient(dividingBy divisor: Digit) -> Self {
        let qr = quotientAndRemainder(dividingBy: divisor); self = Self(digit: qr.remainder); return qr.quotient
    }
